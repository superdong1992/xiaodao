...
from .platform import _chmod_no_follow
        ...
        with self._coordination_lock:
            ...
            try:
                ...
                if original_mode is not None and candidate.exists():
                    _chmod_no_follow(candidate, original_mode)
                raise
            self._file_sync.sync_directory(candidate.parent)
            self._file_sync.sync_directory(destination.parent)
            return destination
    ...
    def delete(self, quarantined_path: Path) -> None:
        ...
        if stat.S_ISREG(target_metadata.st_mode):
            try:
                ...
                _chmod_no_follow(target, stat.S_IRUSR | stat.S_IWUSR)
                target.unlink()
            ...
            for current, directory_names, _ in os.walk(
                ...
                _chmod_no_follow(current, stat.S_IRWXU)
                ...
                for directory_name in directory_names:
                    ...
                    if stat.S_ISDIR(child_metadata.st_mode) and not is_reparse_point(
                        ...
                        _chmod_no_follow(child, stat.S_IRWXU)
            ...
            def make_writable_and_retry(
                ...
                if parent != Path(path):
                    _chmod_no_follow(parent, stat.S_IRWXU)
                _chmod_no_follow(path, stat.S_IRWXU)
                function(path)
