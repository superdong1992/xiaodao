...
from tests.deterministic.unit.interfaces.helpers import problem_spec_input


CREATE_CASE = "problem_locator_create_case"
REMOTE_GATE = "PROBLEM_LOCATOR_NATIVE_CLIENT_LINUX_GATE"
LEGACY_WINDOWS_REMOTE_GATE = "PROBLEM_LOCATOR_WINDOWS_LINUX_GATE"
LINUX_CLIENT_GATE = "PROBLEM_LOCATOR_LINUX_CLIENT_GATE"
...
def _skip_unless_native_client_gate(name: str, reason: str) -> None:
    ...
    enabled = (
        ...
        and (
            ...
            or (
                name == REMOTE_GATE
                and system == "Windows"
                and os.environ.get(LEGACY_WINDOWS_REMOTE_GATE) == "1"
            )
        )
    ) or (system == "Linux" and os.environ.get(LINUX_CLIENT_GATE) == "1")
    if enabled:
        return
...
def test_native_client_direct_http_to_real_linux_mcp_uses_only_flat_inputs() -> None:
    _skip_unless_native_client_gate(
        REMOTE_GATE,
        "requires the explicit native-Client-to-Linux HTTP release gate",
    )
