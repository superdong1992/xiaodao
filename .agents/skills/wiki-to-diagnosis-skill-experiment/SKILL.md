...
2. 从 Wiki 中识别能够凭独立证据确认的定位方法。方法按“判断一种原因所需的证据和计算”拆分，不按标题数量或日志数量机械拆分。
3. 把所有方法都要遵守的观测限制、安全语义和证据边界提取为共享引用，避免在每张卡里重复。
4. 为每种方法选择 Wiki 原文中真实存在的简短日志标记。标记用于先扫描证据、再按需加载方法卡；不得发明 Wiki 中没有的标记。
5. 不能区分具体原因、但用于确认问题发生或关联目标请求的稳定日志模板，放入共享引用。不得因为它不是分支标记就从生成物中丢失。
6. 按用户指定的目录和名称生成一个 Skill。生成前先阅读 [输出合同](references/output-contract.md)，严格使用其中的文件结构和字段。
7. 生成后运行本 Skill 的校验脚本。校验失败时只修正被报告的结构问题；不要借机改变 Wiki 语义。
...
生成的定位 Skill 消费已经冻结的 Logparse `target_logs` 和 receipt：
