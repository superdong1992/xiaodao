...
通用定位会自动分析随问题提交的日志，不要求填写时间、槽位、进程或 PID，也不调用 Intake 提取这些参数。分析开始后补交有效日志，会停止旧 Job，在同一 Case 和同一轮次重新分析；新日志替换旧包，原始问题保留，随日志发送的文字作为补充描述。请求重放不会重复启动。若旧报告先完成提交，本次重启返回 `409`，日志文件仍保留，但不会发布该消息的 `message.accepted`。专用定位的附件采用规则不变。
