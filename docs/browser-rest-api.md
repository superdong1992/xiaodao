...
| `RESOLVED` | 是 | 展示 `final_result`、legacy `generic_result` 或 `generic_result_v2.report_markdown`；重新列出产物并下载、校验。 |
| `PARTIALLY_RESOLVED` | 是 | 展示部分结论、未满足的完成条件与限制；重新列出产物并下载、校验。 |
| `UNRESOLVED` | 是 | 展示 `unresolved_result`、legacy `generic_result` 或 `generic_result_v2.report_markdown`；列出可用结果与审计产物。 |
| `FAILED` | 是 | 展示 `failure.code` 和安全 `message`；不要自动创建替代 Case。 |
...
`generic_result` 是冻结的 legacy V1 结构，`generic_result_v2` 是 V2 原生 Markdown 结构，二者严格互斥。V2 结果必须同时出现一项 `kind=GENERIC_REPORT`、`content_type=text/markdown` 的公开产物；前端应以 `report_artifact_id` 关联它，并验证下载字节与 `report_utf8_size`、`report_sha256` 以及 `report_markdown` 的 UTF-8 字节完全一致。损坏或缺失的绑定不能由前端降级为 legacy V1。
...
type ArtifactKind = "USER_RESULT" | "USER_RESULT_ARCHIVE" | "GENERIC_REPORT" | "DIAGNOSTIC_EXPORT" | "LOGPARSE_RUN" | "AUDIT_BUNDLE";
...
| `CaseView` | `final_result` | `CandidateConclusion \| null` | 专用流程接受的最终结果，仅解决终态存在。 |
| `CaseView` | `unresolved_result` | `UnresolvedResult \| null` | 专用流程未解决结果，仅 `UNRESOLVED` 存在。 |
| `CaseView` | `generic_result` | `GenericResult \| null` | legacy V1 通用流程终态结果；与 `generic_result_v2` 互斥。 |
| `CaseView` | `generic_result_v2` | `GenericResultV2 \| null` | V2 原生 Markdown 通用结果；与 `generic_result` 互斥并绑定公开报告产物。 |
...
| `GenericResultV2` | `format_version` | literal `2` | V2 Markdown 报告合同版本。 |
| `GenericResultV2` | `status` | `GenericResultStatus` | 通用流程终态，必须与 Case 状态一致。 |
| `GenericResultV2` | `report_markdown` | `text` | 完整原生 Markdown；服务端不 trim、不归一化。 |
| `GenericResultV2` | `report_utf8_size` | `integer >= 0` | `report_markdown` 的精确 UTF-8 字节数。 |
| `GenericResultV2` | `report_sha256` | `sha256` | `report_markdown` UTF-8 字节的 SHA-256。 |
| `GenericResultV2` | `report_artifact_id` | `uuid` | 同字节公开 `GENERIC_REPORT` 产物。 |
| `GenericResultV2` | `skill_name` | `string` | 执行定义名称，`^[a-z0-9]+(?:-[a-z0-9]+)*$`，最多 64 字符。 |
| `GenericResultV2` | `source_job_id` | `uuid` | 来源 Job。 |
| `GenericResultV2` | `source_outcome_id` | `uuid` | 来源结果事件。 |
| `GenericResultV2` | `occurred_at` | `timestamp` | 产生时间。 |
...
| `ArtifactKind` | `USER_RESULT`, `USER_RESULT_ARCHIVE`, `GENERIC_REPORT`, `DIAGNOSTIC_EXPORT`, `LOGPARSE_RUN`, `AUDIT_BUNDLE` | 结果 JSON、结果归档、V2 通用 Markdown 报告、诊断导出、内部目录、审计包；列表端点只返回公开可下载项。 |
