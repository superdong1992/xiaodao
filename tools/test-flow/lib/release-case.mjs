...
function loadDriver(scenario) {
  ...
  for (const attachment of driver.attachment_files) {
    ...
    resolveOwnedPath(path.dirname(scenario.driver_path), attachment);
  }
  return driver;
}
...
export function loadReleaseCase(caseRoot) {
  ...
  return {
    ...
    approved_skill_dir: resolveOwnedPath(verified.root, descriptor.approved_skill_dir, { directory: true }),
...
export function loadReleaseCaseInputs(caseRoot) {
  ...
  return {
    ...
    scenarios: loaded.scenarios.map((scenario) => {
      const driver = loadDriver(scenario);
      return {
        scenario_id: scenario.scenario_id,
        driver,
