...
test("checkpoint extraction populates nested read-only artifact directories before restoring modes", () => {
  ...
  try {
    ...
    const sourceModes = {
      tree: fs.statSync(tree).mode & 0o777,
      payload: fs.statSync(payload).mode & 0o777,
      result: fs.statSync(result).mode & 0o777,
    };
    if (process.platform !== "win32") assert.deepEqual(sourceModes, { tree: 0o555, payload: 0o555, result: 0o444 });
    ...
    assert.equal(fs.statSync(path.join(target, path.relative(state, tree))).mode & 0o777, sourceModes.tree);
    assert.equal(fs.statSync(path.join(target, path.relative(state, payload))).mode & 0o777, sourceModes.payload);
    assert.equal(fs.statSync(path.join(target, path.relative(state, result))).mode & 0o777, sourceModes.result);
