...
test("the guide tables cover every published field, Case state, and public error code", () => {
  ...
  assert.deepEqual(
    Object.keys(schemas.GenericResultV2?.properties ?? {}).sort(),
    [
      "format_version",
      "occurred_at",
      "report_artifact_id",
    ...
    "OpenAPI must publish the complete GenericResultV2 response model",
  );
  assert.ok(schemas.ArtifactKind?.enum?.includes("GENERIC_REPORT"));
...
test("the TypeScript example implements large-file integrity and actionable recovery", () => {
  ...
  assert.match(guide, /interface GenericResultV2\s*\{/);
  assert.match(guide, /type ArtifactKind\s*=\s*[\s\S]*?"GENERIC_REPORT"/);
