...
test("INTAKE-only Python gate does not reject unrelated framework platform skips", () => {
  const { gates } = JSON.parse(fs.readFileSync(new URL("../config/gates.v2.json", import.meta.url), "utf8"));
  const framework = gates["framework.node-tests"];
  const intake = gates["framework.intake-usage"];
  const { stages } = JSON.parse(fs.readFileSync(new URL("../config/stages.v2.json", import.meta.url), "utf8"));
  assert.ok(stages.find((stage) => stage.id === "framework.self-test").gates.includes("framework.intake-usage"));
  assert.equal(framework.python_driver, undefined);
...
test("the INTAKE gate fails closed before launching Node when its Python runtime is missing", async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "test-flow-intake-missing-python-"));
  const priorPython = process.env.TEST_FLOW_PYTHON;
  try {
    process.env.TEST_FLOW_PYTHON = path.join(root, "missing-python");
    const gate = JSON.parse(fs.readFileSync(new URL("../config/gates.v2.json", import.meta.url), "utf8")).gates["framework.intake-usage"];
    const result = await executeGate({ repoRoot: root, attemptRoot: path.join(root, "attempt") },
      { id: "framework.self-test" }, "framework.intake-usage", gate);
    assert.equal(result.status, "BLOCKED");
    assert.equal(result.failure_domain, "INFRA");
    assert.equal(result.code, "NODE_TEST_PYTHON_DRIVER_MISSING");
  } finally {
    if (priorPython === undefined) delete process.env.TEST_FLOW_PYTHON;
    else process.env.TEST_FLOW_PYTHON = priorPython;
    fs.rmSync(root, { recursive: true, force: true });
  }
});
