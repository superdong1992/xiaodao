...
`det.evidence-v2-core`（保留旧 Gate ID），并生成绑定 source snapshot、V11 contract manifest、固定 Methods V1 Core 用例和
JUnit 的 `core-verdict.json` 子收据；外层 `verdict.json` 仍是唯一权威结论。Release 认证始终在
当前 attempt 重新运行 `deterministic.full`，不会复用历史 Core Stage。`det.website-example` 同时运行网站 TypeScript 后端示例的确定性测试，覆盖登录、会话归属、SSE 转发和报告下载校验；示例源码也纳入证明身份。
