...
> Historical migration material and a CrossJob adapter implementation detail.
> Do not start a new test activity from these commands. Use
> [`../run.ps1`](../run.ps1) or [`../run.sh`](../run.sh); only a registered,
> identity-bound adapter may invoke this bundle.
...
This template drives only the public Windows Claude Code → Remote MCP → HTTP upload path. It never reads, copies, prints, or materializes credential settings. Claude Code itself receives `--setting-sources user,project`; the MCP configuration is an inline, strict, credential-free JSON argument. The v1.0.5 migration requires both scopes to be free of old Problem Locator Hook groups before this gate runs.
...
`-Mode All` runs the same three gates sequentially. Phase 1 creates one Case through the flat input contract, validates parameter group A as an exact name/kind set independent of the randomly sorted requirement array, supplies it through paired scalar arrays, waits for the real `log_archive` requirement, and prepares the descriptor. Upload validates the fixed 2367-byte ZIP and its SHA-256, validates the descriptor exactly, and invokes native `curl.exe` with its four headers. Phase 3 explicitly submits the READY attachment, supplies `order_id`, observes `REVIEWING` then `RESOLVED`, and requires exactly two public artifacts: `diagnosis-result.json` (`USER_RESULT`) and `result.zip` (`USER_RESULT_ARCHIVE`). No Problem Locator client Hook or client DFX is installed; the accepted `LOGPARSE_RUN` remains internal.
