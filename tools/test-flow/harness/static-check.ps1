...
$targetNodes = @(
    'tests/deterministic/unit/application/test_external_commands.py::test_submit_supplement_accepts_canonical_fact_order_for_multiple_inputs',
    'tests/deterministic/unit/integrations/test_logparse_primitives.py::test_git_inventory_trusts_only_the_exact_configured_repository',
    'tests/deterministic/unit/integrations/test_logparse_primitives.py::test_git_inventory_ignores_ambient_repository_and_config_redirection',
    'tests/deterministic/unit/integrations/test_logparse_primitives.py::test_git_inventory_rejects_a_safe_directory_wildcard_path'
)
...
if ($preGatesText -match '(?m)^\s*tests/deterministic/unit/integrations/test_logparse_primitives\.py\s*\\\s*$') {
...
$expectedPatchHash = ([System.IO.File]::ReadAllText((Join-Path $DriverRoot 'source.patch.sha256')) -split '\s+')[0]
if ((Get-FileHash -LiteralPath (Join-Path $DriverRoot 'source.patch') -Algorithm SHA256).Hash.ToLowerInvariant() -cne $expectedPatchHash) {
    throw 'source patch digest must match its frozen receipt'
}
$patchFiles = @([System.IO.File]::ReadAllLines((Join-Path $DriverRoot 'source.patch.files.txt')))
$canonicalPatchFiles = [string[]]@($patchFiles | Sort-Object -Unique)
if ($patchFiles.Count -eq 0 -or $patchFiles.Count -ne $canonicalPatchFiles.Count) {
    throw 'source patch inventory must be non-empty and unique'
}
for ($patchIndex = 0; $patchIndex -lt $canonicalPatchFiles.Count; $patchIndex++) {
    if ($patchFiles[$patchIndex] -cne $canonicalPatchFiles[$patchIndex]) {
        throw "source patch inventory must be ordinal-sorted at index $patchIndex"
    }
    if ($patchFiles[$patchIndex] -notmatch '^[A-Za-z0-9._/-]+$') {
        throw "source patch inventory contains an unsafe path at index $patchIndex"
    }
}
