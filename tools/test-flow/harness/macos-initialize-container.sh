...
if [ "$mode" = fresh ]; then
  ...
  /opt/venvs/xiaodao/bin/python -I /harness/prepare_real_zip.py >/dev/null
else
  test -n "$(find /var/lib/problem-locator -mindepth 1 -print -quit)"
  test -f /evidence/synthetic-rpc-service-takeover.zip
fi

/opt/venvs/xiaodao/bin/python -I /harness/prepare_claude_settings.py >/dev/null
...
/opt/venvs/xiaodao/bin/python -I /harness/prepare_nonroot_settings.py create >/dev/null
chown 10001:10001 /run/plagent-claude/settings.json
/opt/venvs/xiaodao/bin/python -I /harness/prepare_nonroot_settings.py verify
