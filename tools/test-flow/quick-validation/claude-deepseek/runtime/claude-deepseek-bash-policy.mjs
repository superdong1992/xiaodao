...
const STEPS = Object.freeze(["openssl", "stat", "curl"]);
...
function classify(words, policy) {
  ...
  if (words.length === 4 && words[0] === "/usr/bin/openssl" && words[1] === "dgst" && words[2] === "-sha256" && words[3] === policy.archive_path) return { allowed: true, step: "openssl" };
  if (words.length === 4 && words[0] === "/usr/bin/stat" && words[1] === "-f" && words[2] === "%z" && words[3] === policy.archive_path) return { allowed: true, step: "stat" };
  if (words[0] === "/usr/bin/curl") return curlContract(words, policy);
  return denied("Only the frozen openssl, stat, and descriptor-bound curl PUT commands are allowed");
}
...
function claim(policy, step) {
  ...
  if (STEPS.slice(0, index).some((prior) => !fs.existsSync(path.join(policy.claim_root, prior)))) return denied("Allowed Bash commands must execute in openssl, stat, curl order");
